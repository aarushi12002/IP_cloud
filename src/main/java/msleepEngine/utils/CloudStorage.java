...
public class CloudStorage {
	...
	public static void uploadFile(String bucketName, String filePath)
		...
		try {
			...
			InputStreamContent content = new InputStreamContent(contentType,
					stream);
		//	Storage.Objects.Insert(java.lang.String bucket, StorageObject content, com.google.api.client.http.AbstractInputStreamContent mediaContent)
	...
	public static void downloadFile(String bucketName, String fileName, String destinationDirectory) throws Exception {
		...
		if(!directory.isDirectory()) {
			throw new Exception("Provided destination Directory path is not a directory");
		}
	...
	public static void createBucket(String bucketName) throws Exception {
		...
		storage.buckets()
				.insert(AppConfig.getHandle().getString(PROJECT_ID_PROPERTY),
						bucket).execute();
	}
	...
	public static List<String> listBuckets() throws Exception {
		...
		List<Bucket> buckets = storage.buckets().list(AppConfig.getHandle().getString(PROJECT_ID_PROPERTY)).execute().getItems();
		if(buckets != null) {
...
//	}
	...
	private static Storage getStorage() throws Exception {
		...
		if (storage == null) {
			...
			System.out.println("curRRRRRR"+"user.dir");
			...
			System.out.println("account id prop "+AppConfig.getHandle().getString(
					ACCOUNT_ID_PROPERTY));
			System.out.println("we reached here first");
			Credential credential = new GoogleCredential.Builder()
					.setTransport(httpTransport)
					.setJsonFactory(jsonFactory)
					.setServiceAccountId(
							AppConfig.getHandle()
									.getString(ACCOUNT_ID_PROPERTY))
					.setServiceAccountPrivateKeyFromP12File(
							new File(AppConfig.getHandle().getString(
									PRIVATE_KEY_PATH_PROPERTY)))
					.setServiceAccountScopes(scopes).build();

			storage = new Storage.Builder(httpTransport, jsonFactory,
					credential).setApplicationName(
					AppConfig.getHandle().getString(APPLICATION_NAME_PROPERTY))
					.build();
		}
		System.out.println("this is the storage"+ storage);
		return storage;
	}
